...
export default function VehiclesPage() {
  ...
  const load = (due: boolean) => {
    setLoading(true);
    vehiclesApi.list(due || undefined).then(setVehicles).finally(() => setLoading(false));
  };

  useEffect(() => { load(dueOnly); }, [dueOnly]);
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: "var(--steel)", fontSize: 14 }}>{vehicles.length} kendaraan {dueOnly ? "harus servis" : "tercatat"}</p>
        </div>
        <button className="btn-rust" onClick={() => setShowAdd(true)}><Plus size={16} /> Tambah Kendaraan</button>
      </div>

      <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
        <button onClick={() => setDueOnly(false)} className={dueOnly ? "btn-ghost" : "btn-rust"} style={{ fontSize: 13 }}>Semua</button>
        <button onClick={() => setDueOnly(true)} className={dueOnly ? "btn-rust" : "btn-ghost"} style={{ fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <tbody>
              ...
              {vehicles.length === 0 && (
                <tr><td colSpan={6} style={{ textAlign: "center", padding: 32, color: "var(--steel)" }}>
                  {dueOnly ? "Tidak ada kendaraan yang harus servis" : "Belum ada kendaraan"}
                </td></tr>
              )}
            </tbody>
          </table>
